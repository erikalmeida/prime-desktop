...
import ucla.si.servicio.ServicioEspacio;
	...
	public class ControladorAsignarEspacioCatalogo  extends ControladorInicio {
		...
		protected void inicializar() {
			//ListModelList<Espacio> modeloEspacios = new ListModelList<Espacio>(servicioEspacio.listarEspaciosPorAsignar());
			...
			if (!listEspacios.getItems().isEmpty()) {
				for (Listitem listitem : listItemEspacios) {
					if (listitem.isSelected()) {
						...
						//	String dir = "gc/espacio/frm-configurar-espacio-cita.zul";
							clearDivApp(dir);
							//
						}
					}
		...
		@Listen("onClick =#btnAsignarCita")
		public void annadirAsignarCita() {
			System.out.println("entro bien");
			List<Listitem> listItemEspacios = new ArrayList<Listitem>(listEspacios.getItems());
			// Clients.evalJavaScript("document.title = 'ServiAldanas'; ");
			if (!listEspacios.getItems().isEmpty()) {
